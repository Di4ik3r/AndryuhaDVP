...
public class GifDecoder {
    ...
    public ArrayList<TextureRegion> getFrames() {
        ...
        Texture texture = new Texture(target);
        ArrayList<TextureRegion> texReg = new ArrayList<TextureRegion>();
...
//        Animation<TextureRegion> result = new Animation<TextureRegion>(frameDuration, texReg, playMode);
    ...
    public static ArrayList<TextureRegion> generateFrames(InputStream is) {
        GifDecoder gdec = new GifDecoder();
        gdec.read(is);
        return gdec.getFrames();
    }

    // EXAMPLE
    // this.cutScene = new CutScene(GifDecoder.generateFrames(Gdx.files.internal("water.gif").read()), 1, true);
}
